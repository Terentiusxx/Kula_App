...
const appLogger = require("firebase-functions/logger");
...
exports.notifyMessageRecipients = onDocumentCreated(
    ...
    async (event) => {
      ...
      if (!chatId || !messageId || !senderId || !messageText) {
        appLogger.warn("Skipping notification: missing message fields", {
          chatId,
          messageId,
        });
        return;
      ...
      if (!chatDoc.exists) {
        appLogger.warn("Skipping notification: chat doc missing", {chatId});
        return;
      }
      ...
      if (!senderName || !senderAvatar) {
        const senderProfileDoc = await db
            .collection("users")
            .doc(senderId)
            .get();
      ...
      recipients.forEach((userId) => {
        const notificationId =
          "msg_" + chatId + "_" + messageId + "_" + userId;
